...
A couple of things need to be said here. Overall the `bestNormalize()` function returns the best values; but we cannot use this for the response variable `sale_price` because we need the log-transform to assess our model. 
...
The competition on Kaggle is [evaluated](https://www.kaggle.com/competitions/house-prices-advanced-regression-techniques/overview/evaluation) using the RMSE between the predicted value and the sale price
